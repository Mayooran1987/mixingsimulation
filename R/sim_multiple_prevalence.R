...
##' UDL <- 138
##' n_sim <- 20000
##' sim_multiple_prevalence(mu, sigma , alpha_in, k, l, rate, distribution, UDL, n_sim)
##' @export
sim_multiple_prevalence <- function(mu, sigma , alpha_in, k, l, rate, distribution, UDL, n_sim){
  # Total_CFU <- NULL
  # mixing_scheme <- NULL
  ...
  for (j in 1:length(distribution)) {
    sim.sum3[,j] <-  sim_single_prevalence(mu, sigma , alpha_in, k[j], l, rate, distribution[j], UDL, n_sim)
  }
