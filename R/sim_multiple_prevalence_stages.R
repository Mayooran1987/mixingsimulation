...
##' @details Let \eqn{N'} be the number of colony-forming units in the mixed sample which is produced by contribution of \eqn{k} primary samples mixing, \eqn{N' = \sum N_i} and \eqn{l} be the number of stages in the mixing process. This function estimates the average prevalence value after a specific number of revolutions in each mixing scheme. However, we need to apply function \link{sim_single_prevalence_stages} if we want to estimate individual prevalence values at each stage of the mixing process.
...
##' UL <- 138
##' n_sim <- 20000
##' sim_multiple_prevalence_stages(mu, sigma , alpha_in, k, l, rate, distribution, UL, n_sim)
##' @export
sim_multiple_prevalence_stages <- function(mu, sigma , alpha_in, k, l, rate, distribution, UL, n_sim){
  # Total_CFU <- NULL
  # mixing_scheme <- NULL
  ...
  for(j in 1:length(distribution)){
    sim.sum3[,j] <-  sim_single_prevalence_stages(mu, sigma , alpha_in, k[j], l, rate, distribution[j], UL, n_sim)
  }
  result <-  apply(sim.sum3, 2, mean)
