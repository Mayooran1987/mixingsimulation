...
##' @title Graphical comparison of mixing plans based on the estimated detection probability at each revolution.
...
##' @return graphical display of estimated detection probability at each revolution in the mixing.
...
compare_mixing_1 <-  function(mu,sigma , alpha_in, k, l, r, distribution, UDL, n_sim){
  ...
  Revolutions <- 1:l
  result <- data.frame(Revolutions, sim.sum3)
  colnames(result) <- c("Revolutions", f_spri(k, distribution))
  melten.Prob <- reshape2::melt(result, id = "Revolutions", variable.name = "mixing_scheme", value.name = "prob.detection")
  plot1 <- ggplot2::ggplot(melten.Prob, ggplot2::aes(prob.detection, group = mixing_scheme, colour = mixing_scheme)) +
    ggplot2::geom_line(ggplot2::aes(x = Revolutions, y = prob.detection)) +
